...
const Input: React.ForwardRefExoticComponent<InputProps & React.RefAttributes<HTMLInputElement>> = React.forwardRef<HTMLInputElement, InputProps>(
    ...
    return (
      <div className="flex h-min w-full flex-col items-center gap-1">
        ...
        <div className="relative flex h-10 w-full items-center rounded-3xl">
          <div className="absolute inset-y-0 left-0 flex h-10 w-10 items-center justify-center bg-transparent pl-2 text-3xl">{icon}</div>
          <input
            {...props}
            ref={ref}
            className={cn(
